...
namespace VoteWithYourWallet.Controllers
{
    public class CauseController : Controller
    {
        ...
        {
            var cause = db.Causes.Find((int)id);
            if (cause == null)
            {
                return NotFound();
            }
            ...
            var detailsModel = new CauseDetailsModel();
            detailsModel.Cause = cause;
            detailsModel.CurrentURL = Request.Host + Request.Path;
        ...
        {
            Cause cause = db.Causes.Find(int.Parse(causeId));
            JsonResponseModel model = null;
            if (cause == null)
            {
                model = new JsonResponseModel() { status = false, msg = "Failed Due To Cause Does Not Exist" };
                return new JsonResult(model);
            }

            if (db.Signatures.Any(a => a.CauseId == cause.Id && a.IdentityUserId == userId))
            {
                model = new JsonResponseModel() { status = false, msg = "Signature Already Exists" };
                return new JsonResult(model);
            }
            ...
            model = new JsonResponseModel() { status = true, msg = "Successfully Signed Up To Cause" };
            return new JsonResult(model);
        }
        ...
        public JsonResult GetSignatures(string causeId)
        {
            ...
            {
                model = new JsonResponseModel() { status = false, msg = "No Signatures" };
                return new JsonResult(model);
            }
            Cause cause = db.Causes.Find(int.Parse(causeId));
            if (cause != null)
            {
                List<JsonResponseModel> list = new List<JsonResponseModel>();
                foreach (var signature in allSignatures)
                {
                    list.Add(new JsonResponseModel() { msg = userManager.FindByIdAsync(signature.IdentityUserId).Result.UserName });
                }
                model = new JsonResponseModel() { status = true, msg = JsonConvert.SerializeObject(list) };
                return new JsonResult(model);
            }
            else
            {
